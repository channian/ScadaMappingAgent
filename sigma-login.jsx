...
function SigmaLogin({ onSuccess }) {
  ...
  const requestOtp = () => {
    ...
    if (!/^[A-Z]?\d{4,8}$/i.test(empId.trim())) { setErr('工號格式不正確（範例：E12345）'); return; }
    setBusy(true);
    setTimeout(() => {
      const mockEmail = `${empId.toLowerCase()}@sigma-corp.com`;
      setEmail(mockEmail);
      setStep('otp');
      setBusy(false);
      setCooldown(60);
  ...
  const verifyOtp = () => {
    ...
    setTimeout(() => {
      setBusy(false);
      onSuccess({ empId, email, name: '陳工程師' });
    }, 700);
  };
        ...
        <aside className="sigma-login-aside">
          <div className="aside-content">
            ...
            <div className="aside-hero">
              ...
              <p>上傳含 <code style={{fontFamily: 'var(--font-mono)', color: 'var(--accent-2)', background: 'rgba(34,211,238,0.1)', padding: '1px 6px', borderRadius: 4}}>tag_name</code> 的 CSV / Excel，系統會從 SCADA iolist 主檔自動補齊 I/O Device、Address、Scale 範圍，產出可直接匯入 Kepware 的格式。</p>
            </div>
          ...
          <div className="aside-footer">
            ...
            <span>SIGMA TAG · v1.0.0 · build 2026.05.27</span>
            ...
            {step === 'empid' && (
              <>
                ...
                <div className="security-notes">
                  ...
                  <div className="item"><Icon.alert />若您忘記工號或信箱未綁定，請洽 IT (#sigma-tag-help)。</div>
                </div>
              </>
            )}
            ...
            {step === 'otp' && (
              <>
                ...
                <div className="email-sent">
                  <Icon.send />
                  <div>
                    OTP 已寄至 <span className="e">{email}</span>
                  </div>
                </div>
